...
export default function SaveUnsave() {
  ...
  const ResetList = () => {
    dispatch(actions.unsaveTask())
    dispatch(actions.setJob(""));
  };
  return (
    <>
      <button
        className=" absolute bottom-24 right-10 bg-green-500 hover:bg-green-400 text-white font-bold py-2 px-4 border-b-4 border-green-700 hover:border-green-500 rounded"
        onClick={SaveList}
      >
        Save
      </button>
